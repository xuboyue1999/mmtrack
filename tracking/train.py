...
def parse_args():
    ...
    parser.add_argument('--port', type=int, default='19999', help='Port of the current rank 0.')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "CUDA_VISIBLE_DEVICES=3 python -W ignore lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d "\
        ...
        train_cmd = "CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node %d --master_port %d lib/train/run_training.py "\
        ...
        train_cmd = "CUDA_VISIBLE_DEVICES=0,1 python -W ignore -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py "\
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d "\
                    "--distill %d --script_teacher %s --config_teacher %s"\
