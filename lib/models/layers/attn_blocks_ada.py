...
def candidate_elimination_prompt(tokens: torch.Tensor, lens_t: int, global_index: torch.Tensor):
    ...
    attentive_tokens = tokens_s.gather(dim=1, index=global_index.long().unsqueeze(-1).expand(B, -1, C))
    tokens_new = torch.cat([query,tokens_t, attentive_tokens], dim=1)
...
class CEBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.du_adapter=DownUp_Adapter2(dim)
