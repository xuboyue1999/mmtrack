...
def LasherDataset():
    return LasherDatasetClass()


class LasherDatasetClass(BaseDataset):
    """LasHeR evaluation dataset."""

    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.lasher_path
        if not self.base_path:
            raise RuntimeError('LasHeR dataset path is not configured. Please set `lasher_path` in lib/test/evaluation/local.py.')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        return Sequence_RGBT(sequence_name, frames, framesi, 'Lasher', ground_truth_rect, ground_truth_recti)
    ...
    def _get_sequence_list(self):
        for candidate in (
            os.path.abspath(os.path.join(self.base_path, '..', 'testingsetList.txt')),
            os.path.join(self.base_path, 'testingsetList.txt'),
        ):
