...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        tir = self._read_image(seq.framesi[0])


        start_time = time.time()


        out = tracker.initialize(image, tir, init_info)
        ...
        for frame_num, frame_path_bind in enumerate(zip(seq.frames[1:],seq.framesi[1:]), start=1):

            image = self._read_image(frame_path_bind[0])
            tir = self._read_image(frame_path_bind[1])
            ...
            out = tracker.track(image, tir, info)

            prev_output = OrderedDict(out)
            _store_outputs(out, {'time': time.time() - start_time})

        for key in ['target_bbox', 'all_boxes', 'all_scores', 'segmentation','confidence']:
    ...
    def _read_image_d(self, image_file: str):
        if isinstance(image_file, str):
            im = cv.imread(image_file)
            dp = cv.normalize(im, None, alpha=0, beta=255, norm_type=cv.NORM_MINMAX)
            dp = np.asarray(dp, dtype=np.uint8)
            image_tir = cv.applyColorMap(dp, cv.COLORMAP_JET)
            return image_tir
        elif isinstance(image_file, list) and len(image_file) == 2:
            return decode_img(image_file[0], image_file[1])
        else:
