...
def rgbd1kDataset():
    return rgbd1kDatasetClass()


class rgbd1kDatasetClass(BaseDataset):
    """RGBD1K evaluation dataset."""

    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.rgbd1k_path
        if not self.base_path:
            raise RuntimeError('RGBD1K dataset path is not configured. Please set `rgbd1k_path` in lib/test/evaluation/local.py.')
    ...
    def _construct_sequence(self, sequence_name):
        anno_path = os.path.join(self.base_path, sequence_name, 'groundtruth_rect.txt')
        try:
            ground_truth_rect = np.loadtxt(anno_path, dtype=np.float64)
        except Exception:
            ground_truth_rect = np.loadtxt(anno_path, delimiter=',', dtype=np.float64)

        anno_pathi = os.path.join(self.base_path, sequence_name, 'groundtruth_rect.txt')
        try:
        ...
        return Sequence_RGBT(sequence_name, frames, framesi, 'rgbd1k', ground_truth_rect, ground_truth_recti)
