...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask','att','tir', 'tbox']
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                ...
                if var_name in ['coords', 'bbox','tbox']:
                ...
                if isinstance(var, (list, tuple)):
                    for i in range(len(var)):
                        try:
                            outputs[var_name] = transform_func(var[i], *params)
    ...
    def _get_image_size(self, inputs):
        im = None
        for var_name in ['image', 'tir','mask']:
    ...
    def transform_tbox(self, bbox, image_shape, *rand_params):
        """Assumes [x, y, w, h]"""
    ...
    def transform_tir(self, tir, *rand_params):
        """Must be deterministic"""
        return tir
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_tir(self, tir, brightness_factor):
        tir = torch.from_numpy(tir.transpose((2, 0, 1)))
        if isinstance(tir, np.ndarray):
            return (torch.from_numpy(tir)/255.0).clamp(0.0, 1.0)
        else:
            return (tir/255.0).clamp(0.0, 1.0)
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale=False):
