...
class TransformBase:
    ...
    def _get_image_size(self, inputs):
        im = None
        for var_name in ['image', 'mask','tir']:
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_tir(self, image, brightness_factor):
...
class ToGrayscale(TransformBase):
    ...
    def transform_tir(self, image, do_grayscale):
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_tir(self, image, do_flip):
        if do_flip:
            if torch.is_tensor(image):
                return image.flip((2,))
            return np.fliplr(image).copy()
