...
class RGBD1K(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if not root:
            raise RuntimeError('RGBD1K dataset path is not configured. Please set `rgbd1k_dir` in lib/train/admin/local.py.')
    ...
    def _get_data_root(self):
        for candidate in ('RGBD1K_train_labelled', 'sequences_RGBD1K'):
            data_root = os.path.join(self.root, candidate)
            if os.path.isdir(data_root):
                return data_root
        return self.root

    def _read_split_list(self, filename):
        list_path = os.path.join(self.root, filename)
    ...
    def _get_sequence_list(self):
        split_to_file = {
            'train': 'rgbd1k_train.txt',
            'test': 'rgbd1k_val.txt',
        }
        if self.split in split_to_file:
            sequence_list = self._read_split_list(split_to_file[self.split])
            if sequence_list is not None:
                return sequence_list

        dir_list = [name for name in os.listdir(self._get_data_root()) if os.path.isdir(os.path.join(self._get_data_root(), name))]
        dir_list.sort()
        return dir_list
    ...
    def _get_sequence_dict(self):
        ...
        for seq in self.sequence_list:
            seq_dict['visible'][seq] = sorted(os.listdir(os.path.join(data_root, seq, 'color')))
            seq_dict['infrared'][seq] = sorted(os.listdir(os.path.join(data_root, seq, 'depth')))
        return seq_dict

    def _read_bb_anno(self, seq_path):
        gt = pandas.read_csv(
            os.path.join(seq_path, 'groundtruth_rect.txt'),
            delimiter=',',
            header=None,
            dtype=np.float32,
            na_filter=False,
    ...
    def _get_sequence_path(self, seq_id):
        return os.path.join(self._get_data_root(), self.sequence_list[seq_id])
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        img_vis_path = os.path.join(seq_path, 'color', self.seq_img_dict['visible'][seq][frame_id])
        img_tir_path = os.path.join(seq_path, 'depth', self.seq_img_dict['infrared'][seq][frame_id])
