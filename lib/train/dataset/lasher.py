...
class LasHeR(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if not root:
            raise RuntimeError('LasHeR dataset path is not configured. Please set `lasher_dir` in lib/train/admin/local.py.')
    ...
    def _read_split_list(self, filename):
        list_path = os.path.abspath(os.path.join(self.root, '..', filename))
    ...
    def _get_sequence_list(self):
        split_to_file = {
            'train': 'trainingsetList.txt',
            'test': 'testingsetList.txt',
            'enhance': 'trainingsetList_enhance.txt',
        }
    ...
    def _get_sequence_dict(self):
        ...
        for seq in self.sequence_list:
            seq_dict['visible'][seq] = sorted(os.listdir(os.path.join(self.root, seq, 'visible')))
            seq_dict['infrared'][seq] = sorted(os.listdir(os.path.join(self.root, seq, 'infrared')))
        return seq_dict
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        img_vis_path = os.path.join(seq_path, 'visible', self.seq_img_dict['visible'][seq][frame_id])
        img_tir_path = os.path.join(seq_path, 'infrared', self.seq_img_dict['infrared'][seq][frame_id])
