...
class DepthTrack(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if not root:
            raise RuntimeError('DepthTrack dataset path is not configured. Please set `depthtrack_dir` in lib/train/admin/local.py.')
    ...
    def _get_data_root(self):
        ...
        return train_root if os.path.isdir(train_root) else self.root
    ...
    def _get_sequence_list(self):
        split_to_file = {
            'train': 'depthtrack_train.txt',
            'test': 'depthtrack_test.txt',
        }
